...
bool FilePath::isDirectory() const {
    assert(valid, INVALID_STR);
    std::error_code err;
    bool isDir = std::filesystem::is_directory(str().cstr(), err);
    assert(err.value() == 0, "Couldn't check if path is directory (dir: " + str() + "; err: " + err.message() + " (" + PGE::String::from(err.value()) + "))");
    return isDir;
}
...
bool FilePath::exists() const {
    ...
    assert(err.value() == 0, "Couldn't check if directory exists (dir: " + str() + "; err: " + err.message() + " (" + PGE::String::from(err.value()) + "))");
    return exists;
}
...
u64 FilePath::getLastModifyTime() const {
    ...
    assert(err.value() == 0, "Couldn't check directory modify time (dir: " + str() + "; err: " + err.message() + " (" + PGE::String::from(err.value()) + "))");
...
bool FilePath::createDirectory() const {
    ...
    assert(err.value() == 0, "Couldn't create directory (dir: " + str() + "; err: " + err.message() + " (" + PGE::String::from(err.value()) + "))");
    return created;
}
...
const std::vector<byte> FilePath::readBytes() const {
    ...
    assert(file.is_open(), "Couldn't read bytes from file (file: \"" + str() + "\")");
