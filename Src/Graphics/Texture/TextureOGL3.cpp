...
TextureOGL3::TextureOGL3(const Graphics& gfx, int w, int h, Format fmt) : Texture(w, h, true, fmt), resourceManager((GraphicsOGL3&)gfx) {
...
TextureOGL3::TextureOGL3(const Graphics& gfx, int w, int h, const byte* buffer, Format fmt, bool mipmaps) : Texture(w, h, false, fmt), resourceManager((GraphicsOGL3&)gfx) {
    ((GraphicsOGL3&)gfx).takeGlContext();
    glTexture = resourceManager.addNewResource<GLTexture>();
