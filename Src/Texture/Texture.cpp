...
static BYTE* load(FIBITMAP* data, const String& name, int& width, int& height, int& realWidth, int& realHeight) {
    __ASSERT(data != nullptr, "Failed to load " + name);

    FIBITMAP* image = FreeImage_ConvertTo32Bits(data);
    __ASSERT(image != nullptr, "Failed to convert " + name + " to 32-bit RGBA");
    ...
    if (w != po2w || h != po2h) {
        ...
        __ASSERT(image != nullptr, "Failed to rescale image (filename: " + name + "; dimensions:" + String::fromInt(w) + "x" + String::fromInt(h) + " -> " + String::fromInt(po2w) + "x" + String::fromInt(po2h) + ")");
        FreeImage_Unload(data);
    }
    ...
    __ASSERT(bits != nullptr, "Failed to retrieve data (filename: " + name + ")");
