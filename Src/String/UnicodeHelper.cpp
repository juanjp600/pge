...
#define PGE_ASSERT_CHAR(chr) char32_t PGE_TMP = chr; PGE_ASSERT(PGE_TMP != 0 && PGE_TMP != 0xFFFF && PGE_TMP != 0xFFFE, "Invalid character (" + String::fromInt(PGE_TMP) + ")")
...
char32_t Unicode::utf8ToWChar(const char* cbuffer) {
    return utf8ToWChar(cbuffer, measureCodepoint(*cbuffer));
}

char32_t Unicode::utf8ToWChar(const char* cbuffer, int codepointLen) {
    if (codepointLen == 1) {
        return cbuffer[0];
    } else {
        // Decode first byte by skipping all bits that indicate the length of the codepoint.
        char32_t newChar = cbuffer[0] & (0x7f >> codepointLen);
...
int Unicode::wCharToUtf8(char32_t chr, char* result) {
    PGE_ASSERT_CHAR(chr);
