...
char16 String::BasicIterator::operator*() const {
    assert(index >= 0 && index < ref->byteLength(), "Tried dereferencing invalid iterator");
    if (_ch == L'\uFFFF') {
...
String::Iterator& String::Iterator::operator++() {
    assert(index < ref->byteLength(), "Tried incrementing end iterator");
    increment();
    return *this;
}

String::Iterator& String::Iterator::operator--() {
    assert(index > 0, "Tried decrementing begin iterator");
    decrement();
    return *this;
}

String::ReverseIterator& String::ReverseIterator::operator++() {
    assert(index >= 0, "Tried decrementing end reverse iterator");
    decrement();
    return *this;
}

String::ReverseIterator& String::ReverseIterator::operator--() {
    assert(index < String::Iterator::end(*ref).index, "Tried incrementing begin reverse iterator");
    increment();
    return *this;
}
...
void String::Iterator::validate() {
    assert(index >= 0, INVALID_ITERATOR);
}

void String::ReverseIterator::validate() {
    assert(index < ref->byteLength(), INVALID_ITERATOR);
}
...
int String::byteLength() const {
    assert(data->strByteLength >= 0, "String byte length must always be valid");
...
const String String::replace(const String& fnd, const String& rplace) const {
    assert(fnd.byteLength() != 0, "Find string can't be empty");
