...
class AABBox {
    ...
    public:
        ...
        constexpr bool operator==(const AABBox& other) { return min == other.min && max == other.max; }
        constexpr bool operator!=(const AABBox& other) { return min != other.min || max != other.max; }

        constexpr bool equals(const AABBox& other, float epsilon = Math::EPSILON_DEFAULT) {
